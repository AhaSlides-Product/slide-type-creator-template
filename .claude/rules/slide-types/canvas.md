...
Rendering is a draft, not a finish line. Run the finished canvas through
**`aha-design-canvas-judge`**, fix **every** FAIL, and re-judge until the verdict reads `OK TO SHIP`.
...
- **Judge against the marketplace's `origin/main`, not the local plugin copy.** That
  ...
  `git show origin/main:plugins/aha-design/skills/aha-design-canvas-judge/SKILL.md`,
